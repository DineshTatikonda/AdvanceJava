...
public class DoctorServices {
	...
	public Doctor findDoctorsbyId(int id) throws invalidDoctorIdExection {
		...
		if(!doctor.isPresent()) {
			throw new invalidDoctorIdExection("Doctor ID "+id+" is not valid");
		}
		return doctor.get();
	}
	
	public Doctor findDoctorsbyName(String name) throws invalidDoctorIdExection {
		Optional<Doctor> doctor = doctorRepo.findByName(name);
		if(!doctor.isPresent()) {
			throw new invalidDoctorIdExection("Doctor NAME "+name+" is not valid");
		}
		return doctor.get();
	}
